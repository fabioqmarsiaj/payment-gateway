...
export class FraudService {
  ...
  async processInvoice(invoiceFraudDto: ProcessInvoiceFraudDto) {
    const { invoice_id, account_id, amount } = invoiceFraudDto;
  ...
  async detectFraud(data: { account: Account; amount: number }) {
    ...
    const SUSPICIOUS_TIMEFRAME_HOUR = this.configService.getOrThrow<number>(
      'SUSPICIOUS_TIMEFRAME_HOUR',
    );
    ...
    if (previousInvoices.length) {
      ...
      if (
        amount >
        averageAmount * (SUSPICIOUS_VARIATION_PERCENTAGE / 100) + averageAmount
      ) {
        return {
          hasFraud: true,
    ...
    recentDate.setHours(recentDate.getHours() - SUSPICIOUS_TIMEFRAME_HOUR);
    ...
    if (recentInvoices.length >= SUSPICIOUS_INVOICES_COUNT) {
      return {
        ...
        description: `Account ${account.id} has more than ${SUSPICIOUS_INVOICES_COUNT} invoices in the last ${SUSPICIOUS_TIMEFRAME_HOUR} hours`,
      };
    }
